...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getTime() {
                this.$http.post('/api/data/selectMaxAndMinTimeByIP', {
                    ip: this.ip
             ...
             */
            loopGet() {
                setInterval(this.getRingData, 1000);
                setInterval(this.getLineData, 1000);
            },
            getRingData() {
                var ringdatas = {
                    columns: ['行为', '频数'],
                ...
                this.$http.post('/api/data/selectRtypeByIP', {
                    ip: this.ip
                }, {}).then((response) => {
                    var ringrow = [
                        {'行为': '其他', '频数': 0},
            ...
            getLineData() {
                this.getTime();
                this.$http.post('/api/data/selectRtypeAndIdateByIP', {
                    ip: this.ip
                }, {emulateJSON: true}).then((response) => {
                    var start = new Date(this.value[0]);
                    start.setMinutes(0);
                    start.setSeconds(0);
                    start = this.addDate(start, "h", -1);
                    var end = new Date(this.value[1]);
                    end.setMinutes(0);
                    end.setSeconds(0);
                    end = this.addDate(end, "h", 1);
                    var linedatas = {
                        columns: ['time', '其他', '工作', '游戏', '邮件', '购物', '聊天', '视频'],
                        rows: []
                    };
